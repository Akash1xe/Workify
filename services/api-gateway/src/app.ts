...
app.use("/api/incidents", createProxyMiddleware({
  ...
  changeOrigin: true,
  pathRewrite(path) {
    const stripped = path.replace(/^\/api\/incidents/, "");
    return stripped.startsWith("/") ? stripped : `/${stripped}`;
  },
  on: {
    proxyReq(proxyReq, req) {
      const id = req.headers["x-request-id"];
      if (id) proxyReq.setHeader("x-request-id", id);
      proxyReq.setHeader("x-forwarded-prefix", "/api/incidents");
    },
    error(error, req, res) {
      console.error("Incident service proxy error", {
        requestId: req.headers["x-request-id"],
        message: error.message
      });
      if ("writeHead" in res && !res.headersSent) {
      ...
      if ("end" in res) res.end(JSON.stringify({ error: { code: "UPSTREAM_UNAVAILABLE", message: "Incident service unavailable" } }));
    }
  }
}));

app.use("/api/ingest", createProxyMiddleware({
  target: env.INGESTION_SERVICE_URL,
  changeOrigin: true,
  pathRewrite(path) {
    const stripped = path.replace(/^\/api\/ingest/, "");
    return stripped.startsWith("/") ? stripped : `/${stripped}`;
  },
  on: {
    proxyReq(proxyReq, req) {
      const id = req.headers["x-request-id"];
      if (id) proxyReq.setHeader("x-request-id", id);
      proxyReq.setHeader("x-forwarded-prefix", "/api/ingest");
    },
    error(error, req, res) {
      console.error("Ingestion service proxy error", { requestId: req.headers["x-request-id"], message: error.message });
      if ("writeHead" in res && !res.headersSent) res.writeHead(502, { "content-type": "application/json" });
      if ("end" in res) res.end(JSON.stringify({ error: { code: "UPSTREAM_UNAVAILABLE", message: "Ingestion service unavailable" } }));
    }
  }
}));
